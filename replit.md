...
-   **Display Resolution & Content Adaptation**: Displays can be configured with specific resolutions via dropdown selector. Player automatically adapts video and image content to match configured resolution by applying maxWidth/maxHeight constraints while maintaining responsive fallback (max-w-full max-h-full) for unconfigured displays. Resolution parsing extracts width/height from string format (e.g., "1920x1080") and applies inline styles to media elements. Feature ensures content scales appropriately without overflow while respecting display capabilities.
